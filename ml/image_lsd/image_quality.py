...
# Try importing cv2, provide fallback if cv2 is not available in environment
try:
    import cv2
    HAS_CV2 = True
except ImportError:
...
def validate_image_quality(image_path):
    ...
    if not HAS_CV2:
        # Fallback validation without cv2
        return True, "Success (Basic validation passed)"
    ...
    # A variance below 50 indicates lack of high-frequency edge content (defocused/motion blur)

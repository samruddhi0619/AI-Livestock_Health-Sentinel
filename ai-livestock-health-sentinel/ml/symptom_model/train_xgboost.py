...
def generate_synthetic_data(num_samples=1000):
    ...
    for _ in range(num_samples):
        ...
        # Base symptoms (all zero)
        symptoms = {s: 0.0 for s in SYMPTOMS_LIST}
        
        if disease == "Healthy":
            # Very low chance of random minor symptoms
            ...
            symptoms["swelling"] = 1.0 if np.random.rand() < 0.9 else 0.0 # blisters/foot swelling
            ...
            symptoms["swelling"] = 1.0 if np.random.rand() < 0.9 else 0.0 # Udder swelling
            ...
            symptoms["reduced_activity"] = 1.0 if np.random.rand() < 0.5 else 0.0
            if vaccination == 1:
                # Vaccination reduces chances of brucellosis, so make this sample less probable
                # if vaccination is true, we might swap label or keep it with lower features
...
def main():
    ...
    with open("../models/rf_model.pkl", "wb") as f:
        pickle.dump(rf, f)
    print("Models saved successfully in 'ml/models/'.")
    
    # Evaluate
    for name, model in [(model_type, xgb_model), ("RandomForest", rf)]:
